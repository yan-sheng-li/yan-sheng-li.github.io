...
## 插入中文编码错乱
你现在在 CMD 里执行下面命令（Windows PowerShell 也可以）：
```bash
chcp 65001
type your.sql
```
...
✅ 方法 2：让 MySQL 按 GBK 读取（不用改文件）

如果你不想转编码，也可以让 MySQL 认为文件是 GBK：
```bash
SET NAMES gbk;
SOURCE your.sql;
```
...
```bash
mysql --default-character-set=gbk -u root -p < your.sql
```
